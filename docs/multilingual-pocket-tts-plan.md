...
- [x] **3.4 [DONE]** Produced `pocket-tts-{de,es,pt,it}-q8_0.gguf` (39.2-39.7 dB worst layer, all better than English's 37.2). See `docs/multilingual-pocket-tts-runs.md`. Originally with `--no-encoder`. Validate (`--validate` + SQNR spot-check against English's 37.2 dB worst-layer figure in `docs/pocket-tts/QUANTIZATION.md`).
