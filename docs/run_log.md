...
## Run 9 (2026-03-14) — Parameter investigation: tyger debug

**Commit**: 83c08ab. Q4_0 baseline (2.6 GB). seed=42, noise_temp=0.9, Metal GPU unless noted.
**Text**: "Tyger Tyger, burning bright" with voice=ljspeech.

| File | transition_steps | GPU | Audio | Quality |
|------|-----------------|-----|-------|---------|
| `tyger_ts0.wav` | 0 | Metal | 1.8s | Bad — wrong phonemes |
| `tyger_ts5.wav` | 5 (all trimmed) | Metal | 1.2s | Matches Python but cut early |
| `tyger_novoice.wav` | 0 (no voice) | Metal | 3.3s | Nonsense sounds |
| `tyger_cpu.wav` | 0 | CPU | 1.8s | Identical to tyger_ts0 |
...
## Run 10 (2026-03-14) — Cross-variant quality test: "time" phrase

**Commit**: 83c08ab. voice=ljspeech, noise_temp=0.9, transition_steps=0, seed=42, Metal GPU.
**Text**: "Time is money, who can afford to pay attention?"

| File | Model | GGUF Size | Gen | Audio | RTF |
|------|-------|-----------|-----|-------|-----|
| `time_f32.wav` | F32 | 6.5G | 30.9s | 2.5s | 12.4x |
| `time_q4.wav` | Q4_0 baseline | 2.6G | 7.1s | 2.5s | 2.9x |
| `time_varE.wav` | Var-E (VV-F16 E-Q4) | 1.8G | 6.7s | 2.5s | 2.7x |
| `time_mixed.wav` | Mixed (VV-Q8 E-Q8) | 1.4G | 2.3s | 2.5s | 0.9x |
| `time_varC.wav` | Var-C (VV-Q8 E-Q4) | 1.3G | 2.3s | 2.5s | 0.9x |
...
> **Note**: Samples generated 2026-03-13 22:48–22:57 (commit 4474519) were NOT logged with parameters at the time and cannot be reproduced. Going forward, all runs must be logged with full parameters before or immediately after generation.
