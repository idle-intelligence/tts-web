...
### Phase 2b — Discovery (added mid-run; original Phase 2 hypotheses both refuted)
...
- [x] **5b. Sub-step profiling: where does WASM time go?** (iterate, ~90m). Goal: produce profiling-grade evidence pinpointing the actual WASM bottleneck. Required outputs:
    ...
    4. **Fresh Chrome trace capture**: with the per-step markers in place (use `console.timeStamp` or `performance.mark` so they show up in the trace), run a Playwright headless TADA inference with `chromium.launch({tracesDir: ...})` or use page.tracing API. Save trace to `/tmp/wasm-perf-fresh.json`. Run `scripts/analyze-trace.py` against it and report (a) total gap time, (b) per-step gap distribution, (c) GPU dispatch count per step, (d) any obvious anomalies (long single dispatches, unexpected idle, etc.).
    5. **WebGPU dispatch breakdown**: from the trace, identify the single longest GPU operation. What is it? (matmul on which dim? layernorm? sampling?) Look up the wgpu-pipeline name if visible.
    6. **Hypothesis register**: in `docs/tada/lab-notebook.md`, write a short "Hypotheses for next run" section listing 3-5 ranked candidate root causes with supporting evidence from the profiling. E.g.: "H1: per-step GPU dispatch is dominated by N small Linear matmuls that should be fused", "H2: dequantize-on-the-fly cost from Q4_0 weights is dominant", etc.

    **Convergence criteria**: `docs/tada/lab-notebook.md` contains a "Sub-step profiling" section with: (a) a per-step gen_ms histogram or summary (mean / p50 / p95 / count); (b) a per-frame decode_ms summary; (c) WASM-vs-native side-by-side at this granularity; (d) the longest single GPU op identified by name and duration; (e) a ranked "Hypotheses for next run" subsection with ≥ 3 candidates each cited with profiling evidence.
...
- [x] **6. Land final configuration + write final row** (single-shot, ~45m). After 5b, read all wasm rows from results.md and the new profiling evidence. Three paths — pick based on data:
    ...
    **(d) Diagnose-not-fix path (added 2026-05-08, after Task 4 + Task 5 both refuted)** — if no config crosses the 30% threshold AND 5b's profiling shows the actual root cause requires work outside this run's scope (e.g., kernel fusion in Burn, Q4 dequantize specialization, decoder rewrite), do NOT commit a fix. Use path (c)'s artifacts (wasm-final row, Perf/Quality Tradeoff section) PLUS add a `## Root cause and follow-up` section to lab-notebook that names the root cause from 5b, cites profiling evidence, and lists specific next-run goals (e.g. "Switch decoder to GPU", "Implement fused QKV matmul kernel", "Try Q4_K_M for embed weights"). This path makes "diagnose rigorously" a legitimate convergence outcome — the acceptance criterion's tradeoff branch already covers it.
